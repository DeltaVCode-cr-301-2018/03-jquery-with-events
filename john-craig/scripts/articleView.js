...
articleView.populateFilters = function() {
  $('article').each(function() {
    ...
    if (!$(this).hasClass('template')) {
      ...
      // Todo so, Build an <option> DOM element that we can append to the author <select> element.
...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      ...
      //console.log(author);
      $('article').hide();
      $('article[data-author=\'' + author + '\']').fadeIn('show');
...
articleView.handleCategoryFilter = function() {
  ...
  $('#category-filter').on('change', function() {
    if ($(this).val()) {
      ...
      $('article').hide();
      $('article[data-category=\'' + category + '\']').fadeIn('show');
    } else {
      $('article').show();
      $('article.template').hide();
    }
...
articleView.handleMainNav = function() {
  ...
  $('nav a').on('click', function() {
    ...
    $('.tab-content').hide();
    let activeTab = $(this).parent().data('content');
...
articleView.setTeasers = function() {
  ...
  $('.read-on').on('click', function() {
    ...
    $(this).siblings('.article-body').children('*:nth-of-type(n+2)').show();
